...
public class SubmitRequest30MessageCodec implements IMessageCodec<SubmitRequest> {
    ...
    public SubmitRequest decode(ChannelHandlerContext ctx, int sequenceId,  ByteBuf in) throws Exception {
        ...
        msg.setFeeTerminalId(StringUtils.readString(in, 32, StandardCharsets.US_ASCII));
        short feeTerminalType = in.readByte();
        ...
        for (int i = 0; i < destUsrTl; i++) {
            destTerminalIds[i] = StringUtils.readString(in, 32, StandardCharsets.US_ASCII);
        }
        int destTerminalType = in.readByte();
    ...
    public void encode(ChannelHandlerContext ctx, SubmitRequest msg, ByteBuf out) throws Exception {
        ...
        out.writeBytes(StringUtils.toBytes(msg.getFeeTerminalId(), 32));
        out.writeByte(0); //源终端号码类型，0：真实号码；1：伪码。
        ...
        for (String destTerminalId : msg.getDestTerminalIds()) {
            out.writeBytes(StringUtils.toBytes(destTerminalId, 32));
        }
        out.writeByte(0); //接收短信的用户的号码类型，0：真实号码；1：伪码。
        out.writeByte(msg.getMsgContent().getMsgLength());
        msg.getMsgContent().output(out);
        out.writeBytes(StringUtils.toBytes(null, 20));
    }
